...
try: 
    ...
    from PassivePyCode.PassivePySrc.constants import VERBS_LIST
except: 
    ...
    from PassivePySrc.constants import VERBS_LIST
...
class PassivePyAnalyzer:
        ...
        """
        def __init__(self, nlp:spacy.language.Language=None, spacy_model:str = "en_core_web_lg", verbs_list = VERBS_LIST):

            """
            Create the Detector

            """
            self.nlp, self.matcher = create_matcher(nlp=nlp, spacy_model=spacy_model, verbs_list=verbs_list)
            self.matcher_t = create_matcher_truncated(self.nlp, verbs_list=verbs_list)
            self.matcher_f = create_matcher_full(self.nlp, verbs_list=verbs_list)
